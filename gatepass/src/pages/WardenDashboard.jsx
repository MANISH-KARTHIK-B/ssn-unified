...
export default function WardenDashboard() {
  ...
  function refresh() {
    api.get("/api/warden/gatepass").then((res) => {
      setPasses(res.data);
      setLoading(false);
    });
  }
  ...
  async function decide(id, decision) {
    await api.post(`/api/warden/gatepass/${id}/decision`, { decision });
    refresh();
  }

  const pending = passes.filter((p) => p.approvals.warden === "Pending");
  const decided = passes.filter((p) => p.approvals.warden !== "Pending");
              ...
              {pending.map((p) => (
                <tr key={p.id} className="border-b border-gray-50 last:border-0">
                  ...
                  <td className="px-6 py-4">
                    <span className="rounded-full bg-green-50 px-2 py-0.5 text-[11px] font-medium text-green-700">Approved</span>
                  </td>
                  <td className="px-6 py-4">
        ...
        <div className="divide-y divide-gray-50">
          {decided.slice(0, 10).map((p) => (
            <div key={p.id} className="flex items-center justify-between px-6 py-3 text-sm">
              ...
              <span className={`flex items-center gap-1 text-xs font-semibold ${p.approvals.warden === "Approved" ? "text-green-700" : "text-red-600"}`}>
                <ShieldCheck className="h-3.5 w-3.5" /> {p.approvals.warden}
              </span>
            </div>
          ))}
