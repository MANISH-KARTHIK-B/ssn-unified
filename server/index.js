...
app.get("/api/library/catalog", (req, res) => {
  ...
  res.json(
    db.data.library.catalog.filter(
      (b) =>
        (!q || b.title.toLowerCase().includes(q) || b.author.toLowerCase().includes(q)) &&
        (!genre || genre === "All" || b.genre === genre)
    )
  );
});
...
// Reserve a book that has no copies available right now - joins a waitlist for it.
app.post("/api/library/reserve", requireAuth, (req, res) => {
  const book = db.data.library.catalog.find((b) => b.id === req.body.bookId);
  if (!book) return res.status(404).json({ error: "Book not found" });
  const already = db.data.library.reservations.find((r) => r.bookId === book.id && r.studentId === req.user.id);
  if (already) return res.status(400).json({ error: "You already reserved this book" });
  const reservation = { id: nanoid(8), bookId: book.id, title: book.title, studentId: req.user.id, reservedOn: new Date().toISOString().slice(0, 10) };
  db.data.library.reservations.push(reservation);
  db.write();
  res.json(reservation);
});
// Renew an issued book - pushes the due date out by 7 days, once.
app.post("/api/library/issued/:id/renew", requireAuth, (req, res) => {
  const issue = db.data.library.issued.find((b) => b.id === req.params.id && b.id.startsWith(req.user.id));
  if (!issue) return res.status(404).json({ error: "Issued book not found" });
  if (issue.renewed) return res.status(400).json({ error: "This book has already been renewed once" });
  const newDue = new Date(issue.dueOn);
  newDue.setDate(newDue.getDate() + 7);
  issue.dueOn = newDue.toISOString().slice(0, 10);
  issue.renewed = true;
  db.write();
  res.json(issue);
});
