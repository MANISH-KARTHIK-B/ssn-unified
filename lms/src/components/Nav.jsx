    ...
    <header className="sticky top-0 z-40 border-b border-gray-200 bg-white">
      <div className="mx-auto flex max-w-7xl items-center gap-6 px-6 py-3">
        ...
        <nav className="hidden items-center gap-5 text-sm font-medium text-graphite-500 md:flex">
          <NavLink to="/" end className={({ isActive }) => isActive ? "text-violet-600" : "hover:text-graphite-900"}>Home</NavLink>
          <NavLink to="/" className={({ isActive }) => "hover:text-graphite-900"}>Dashboard</NavLink>
          <NavLink to="/" className={({ isActive }) => isActive ? "text-violet-600" : "hover:text-graphite-900"}>My courses</NavLink>
        </nav>
        <div className="ml-auto flex items-center gap-4">
          <button className="relative text-graphite-500 hover:text-graphite-900" aria-label="Notifications">
            <Bell className="h-5 w-5" />
            <span className="absolute -right-1.5 -top-1.5 grid h-4 w-4 place-items-center rounded-full bg-red-500 text-[10px] font-semibold text-white">1</span>
          </button>
